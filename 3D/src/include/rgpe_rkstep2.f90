...
!$omp parallel do if (kend-ksta.lt.nth) private (i)
           DO j = 1,ny
              DO i = 1,nx
                 R1(i,j,k) = R1(i,j,k) + Vtrap(i,j,k)
              END DO
           END DO
        END DO
        CALL nonlgpe(R1,zre,C3)       ! (|z|^2 + Vtrap/beta).zre
        CALL nonlgpe(R1,zim,C4)       ! (|z|^2 + Vtrap/beta).zim
        CALL laplak3(zre,C5)
        CALL laplak3(zim,C6)
           ...
           DO k = 1,nz
              IF (kn2(k,j,i).le.kmax) THEN
                 C7(k,j,i) = - omegag*zim(k,j,i) + beta*C4(k,j,i)  &
                   - alpha*C6(k,j,i) + C29(k,j,i)    ! store K1
                 C8(k,j,i) =   omegag*zre(k,j,i) - beta*C3(k,j,i)  &
                   + alpha*C5(k,j,i) + C30(k,j,i)    ! store K1
           ...
           DO k = 1,nz
              IF (kn2(k,j,i).le.kmax) THEN
                 C4(k,j,i) = - omegag*zim(k,j,i) + beta*C4(k,j,i)  &
                   - alpha*C6(k,j,i) + C29(k,j,i)    ! K2
                 C5(k,j,i) =   omegag*zre(k,j,i) - beta*C3(k,j,i)  &
                   + alpha*C5(k,j,i) + C30(k,j,i)    ! K2
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
           ...
           DO k = 1,nz
              IF (kn2(k,j,i).le.kmax) THEN
                 C4(k,j,i) = - omegag*zim(k,j,i) + beta*C4(k,j,i)  &
                   - alpha*C6(k,j,i) + C29(k,j,i)    ! K3
                 C5(k,j,i) =   omegag*zre(k,j,i) - beta*C3(k,j,i)  &
                   + alpha*C5(k,j,i) + C30(k,j,i)    ! K3
           ...
           DO k = 1,nz
              IF (kn2(k,j,i).le.kmax) THEN
                 C7(k,j,i) = C7(k,j,i) + (- omegag*zim(k,j,i) + beta*C4(k,j,i) &
                   -alpha*C6(k,j,i) + C29(k,j,i))    ! K1+2.K2+2.K3+K4
                 C8(k,j,i) = C8(k,j,i) + (  omegag*zre(k,j,i) - beta*C3(k,j,i) &
                   +alpha*C5(k,j,i) + C30(k,j,i))    ! K1+2.K2+2.K3+K4
                 zre(k,j,i) = C1(k,j,i) + dt*C7(k,j,i)*rmp
                 zim(k,j,i) = C2(k,j,i) + dt*C8(k,j,i)*rmp
              ELSE
                 zre(k,j,i) = 0.0_GP
