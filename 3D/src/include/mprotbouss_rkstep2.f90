...
!$omp parallel do if (iend-ista.lt.nth) private (k)
         ...
         IF ((trans.eq.1).and.(times.eq.0).and.(bench.eq.0).and.(o.eq.ord)) &
            ...
            CALL sctrans(C21,C6,ext,0)
            CALL sctpara(C21,C6,ext,0)
            CALL sctperp(C21,C6,ext,0)

            CALL sctrans(C22,C24,ext,0)
            CALL sctpara(C22,C24,ext,0)
            CALL sctperp(C22,C24,ext,0)
