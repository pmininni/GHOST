...
! Computes the nonlinear terms and evolves the equations in dt/o
         ...
         CALL gradpressi(gam1,e,vx,vy,vz,C25,C26,C27) ! Grad (p+0.5v^2) term
         CALL vdiss(nu,nu2,vx,vy,vz)              ! viscous term
         CALL divide(rho,vx,vy,vz)                ! viscous term with rho
         CALL divide(rho,C25,C26,C27)             ! divide Grad(p_tot) by rho

         CALL divrhov(rho,vx,vy,vz,C7)            ! div(rho.v)

         CALL divrhov(th,vx,vy,vz,C8)             ! div(e.v)
         CALL pdVwork(gam1,th,vx,vy,vz,C9)        ! p.div(v) = (gamma-1) e.div(v)
         ...
         DO k = 1,nz
            ...
            IF (kn2(k,j,i).le.kmax) THEN
              ...
              +fz(k,j,i))*rmp
               ...
               th (k,j,i) = C21(k,j,i)+dt*(kappa*e(k,j,i)-C8(k,j,i)-C9(k,j,i))*rmp
            ELSE
               vx (k,j,i) = 0.
               vy (k,j,i) = 0.
               vz (k,j,i) = 0.
