...
MODULE class_GPartComm
      ...
      TYPE, PUBLIC :: GPartComm
        ...
        INTEGER,PARAMETER                            :: intrfc_   ! if >=1 use multi-field interface
      ...
      CONTAINS
        ...
        PROCEDURE,PUBLIC :: SlabDataExchange  => GPartComm_SlabDataExchangeMF,GPartComm_SlabDataExchangeSF
        PROCEDURE,PUBLIC :: PDBConcat         => GPartComm_Concat
      ...
      PRIVATE :: GPartComm_PartExchange      , GPartComm_Transpose
...
!-----------------------------------------------------------------
  ...
  SUBROUTINE GPartComm_UnpackSF(this,ve,buff)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!  METHOD     : UnpackSF
!  DESCRIPTION: Unpacks recv buffer with into extended (single) field
...
!    buff        : packed buffer (input) from which to store into 
!                  extended grid quantities.
!    ve          : Eulerian velocity component on extended grid
!                  in phys. space (IN)
!    
!-----------------------------------------------------------------
    USE fprecision
...
!-----------------------------------------------------------------
  ...
  SUBROUTINE GPartComm_PartExchange(this,pdb,nparts,zmin,zmax)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!  METHOD     : PartExchange
...
!-----------------------------------------------------------------
    ...
    this%ntop_ = 0
    DO j = 0, nparts
      IF ( pdb(j)%z.LT.zmin .OR. pdb(j)%z.GT.zmax) THEN ! bottom
        this%ibot_(this%nbot_+1) = j
        this%nbot_ = this%nbot_ + 1
      ENDIF
      IF ( pdb(j)%z.GT.zmax .OR. pdb(j).z.LT.zmin) THEN ! top
    ...
    !
    ! send data:
    CALL GPartComm_PackP(this,this%sbbuff_,this%nbuff_,pdb,nparts,this%ibot_,this%nbot_)
    CALL MPI_ISEND(this%sbbuff_,this%nbuff_,GC_REAL,ibrank, &
                   this%comm_,this%itsh_(1),this%ierr_)
    CALL GPartComm_PackP(this,this%sbbuff_,this%nbuff_,pdb,nparts,this%itop_,this%ntop_)
    CALL MPI_ISEND(this%stbuff_,this%nbuff_,GC_REAL,itrank, &
                   this%comm_,this%itsh_(1),this%ierr_)

    ! Concatenate partcle list to remove particles sent away:
    CALL GPartComm_Concat(this,pdb,nparts,this%ibot_,&
    ...
    CALL GPartComm_UnpackP(this,pdb,nparts,this%rbbuff_,this%nbuff_)
    CALL GPartComm_UnpackP(this,pdb,nparts,this%rtbuff_,this%nbuff_)
...
!-----------------------------------------------------------------
  ...
  SUBROUTINE GPartComm_PackP(this,buff,nbuff,pdb,nparts,iind,nind)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!  METHOD     : PackP
...
!-----------------------------------------------------------------
  ...
  SUBROUTINE GPartComm_Concat(this,pdb,nparts,ibind,nbind,itind,ntind)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!  METHOD     : Concat
!  DESCRIPTION: Removes particles at indices itind,ibind,and
!               concatenates the particles list
!  ARGUMENTS  :
!    this    : 'this' class instance (IN)
!    buff    : buffer into which to pack particles for sends
!    pdb     : part. d.b.
...
!-----------------------------------------------------------------
  ...
  SUBROUTINE GPartComm_UnpackP(this,pdb,nparts,buff,nbuff)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!  METHOD     : UnpackP
...
!$omp parallel do if ((idims(3)-1)/csize.ge.nth) private (jj,kk,i,j,k)
    DO ii = 1,nz,csize
!$omp parallel do if ((idims(3)-1)/csize.lt.nth) private (kk,i,j,k)
       DO jj = 1,ny,csize
          DO kk = 1,nx,csize

             DO i = ii,min(nz,ii+csize-1)
               DO j = jj,min(ny,jj+csize-1)
                 DO k = kk,min(nx,kk+csize-1)
    ...
    ELSE
       ...
       DO ii = 1,ny,csize
         DO jj = 1,nx,csize
            DO i = ii,min(ny,ii+csize-1)
              DO j = jj,min(nx,jj+csize-1)
...
!-----------------------------------------------------------------
    ...
    DO irank = 0,this%nprocs_-1
       ...
       CALL block2d(1,idims(1),jsta,ista,iend,jsta,jend, &
                    GC_REAL,itemp1)
...
!-----------------------------------------------------------------
  ...
  INTEGER FUNCTION GPartComm_GetNumGhost(this) result(nzghost_result)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!  METHOD     : GetNumGhost
