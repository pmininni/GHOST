...
MODULE class_GPSplineInt
      ...
      TYPE, PUBLIC :: GPSplineInt
        PRIVATE
        ! Member data:
        REAL(KIND=GP),ALLOCATABLE,DIMENSION(:,:,:)   :: splfld_,esplfld_
        REAL(KIND=GP),ALLOCATABLE,DIMENSION    (:)   :: ax_,bx_,betx_,cx_,gamx_,px_,xxx_,zetax_
        REAL(KIND=GP),ALLOCATABLE,DIMENSION    (:)   :: ay_,by_,bety_,cy_,gamy_,py_,xxy_,zetay_
        REAL(KIND=GP),ALLOCATABLE,DIMENSION    (:)   :: az_,bz_,betz_,cz_,gamz_,pz_,xxz_,zetaz_
        ...
        INTEGER                                      :: maxint_
        INTEGER                                      :: ierr_,ibnds_(3),ldims_(3),ider_(3),nd_(3)
        INTEGER                                      :: rank_
      ...
      CONTAINS
        ...
        PROCEDURE,PUBLIC :: SetType      => GPSplineInt_SetType
        PROCEDURE,PUBLIC :: SetDeriv     => GPSplineInt_SetDeriv
        PROCEDURE,PUBLIC :: PartUpdate   => GPSplineInt_PartUpdate2D,GPSplineInt_PartUpdate3D
        PROCEDURE,PUBLIC :: CompSpline3D => GPSplineInt_CompSpline3D
        PROCEDURE,PUBLIC :: CompSpline2D => GPSplineInt_CompSpline2D
        PROCEDURE,PUBLIC :: GetType      => GPSplineInt_GetType
...
!-----------------------------------------------------------------
    ...
    IF ( this%rank_ .EQ. 2 .AND. this%ldims_(2) .NE. (this%ibnds_(2,2)=this%this%ibnds_(2,1)+1) ) THEN
      WRITE(*,*) 'GPSplineInt::ctor: Inconsitent 2-indices: rank=2'
      STOP
    ENDIF

    IF ( this%rank_ .EQ. 3 .AND. this%ldims_(3) .NE.  (this%ibnds_(3,2)=this%this%ibnds_(3,1)+1) ) THEN
      WRITE(*,*) 'GPSplineInt::ctor: Inconsitent 3-indices; rank=3'
      STOP
    ENDIF

    CALL GPSplineInt_Init()
...
!-----------------------------------------------------------------
    ...
    CLASS(GPSplineInt)                        :: this
    REAL(KIND=GP),INTENT(INOUT),DIMENSION(*) :: fp
    REAL(KIND=GP)                            :: four,halfm,sixth,six,three,threeh,two
...
!
    ...
    DO lag=1,np
      yy = this%yrk_(lag)
      yym = (1.0-yy)
       this%wrkl_(4,lag) = halfm*yym*yym*this%dyi_(2)
       this%wrkl_(5,lag) = yy*(threeh*yy-two)*this%dyi_(2)
       this%wrkl_(6,lag) = -yym*(threeh*yym-two)*this%dyi_(2)
    ENDDO
    ENDIF
!
...
!
    ...
    DO lag=1,np
      zz = this%zrk_(lag)
      zzm = (1.0-zz)
       this%wrkl_(7,lag) = halfm*zzm*zzm*this%dzi_(3)
       this%wrkl_(8,lag) = zz*(threeh*zz-two)*this%dzi_(3)
       this%wrkl_(9,lag) = -zzm*(threeh*zzm-two)*this%dzi_(3)
    ENDDO
    ENDIF
!
...
!
    DO lag=1,np
      ...
      fp(lag) =
        esplfld_(this%ilg_(1,lag),this%jlg_(1,lag),this%klg_(1,lag))*xx1*yy1*zz1 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(1,lag),this%klg_(1,lag))*xx2*yy1*zz1 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(1,lag),this%klg_(1,lag))*xx3*yy1*zz1 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(1,lag),this%klg_(1,lag))*xx4*yy1*zz1 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(2,lag),this%klg_(1,lag))*xx1*yy2*zz1 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(2,lag),this%klg_(1,lag))*xx2*yy2*zz1 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(2,lag),this%klg_(1,lag))*xx3*yy2*zz1 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(2,lag),this%klg_(1,lag))*xx4*yy2*zz1 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(3,lag),this%klg_(1,lag))*xx1*yy3*zz1 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(3,lag),this%klg_(1,lag))*xx2*yy3*zz1 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(3,lag),this%klg_(1,lag))*xx3*yy3*zz1 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(3,lag),this%klg_(1,lag))*xx4*yy3*zz1 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(4,lag),this%klg_(1,lag))*xx1*yy4*zz1 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(4,lag),this%klg_(1,lag))*xx2*yy4*zz1 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(4,lag),this%klg_(1,lag))*xx3*yy4*zz1 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(4,lag),this%klg_(1,lag))*xx4*yy4*zz1  
c  k = 2
      + esplfld_(this%ilg_(1,lag),this%jlg_(1,lag),this%klg_(2,lag))*xx1*yy1*zz2 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(1,lag),this%klg_(2,lag))*xx2*yy1*zz2 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(1,lag),this%klg_(2,lag))*xx3*yy1*zz2 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(1,lag),this%klg_(2,lag))*xx4*yy1*zz2 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(2,lag),this%klg_(2,lag))*xx1*yy2*zz2 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(2,lag),this%klg_(2,lag))*xx2*yy2*zz2 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(2,lag),this%klg_(2,lag))*xx3*yy2*zz2 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(2,lag),this%klg_(2,lag))*xx4*yy2*zz2 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(3,lag),this%klg_(2,lag))*xx1*yy3*zz2 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(3,lag),this%klg_(2,lag))*xx2*yy3*zz2 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(3,lag),this%klg_(2,lag))*xx3*yy3*zz2 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(3,lag),this%klg_(2,lag))*xx4*yy3*zz2 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(4,lag),this%klg_(2,lag))*xx1*yy4*zz2 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(4,lag),this%klg_(2,lag))*xx2*yy4*zz2 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(4,lag),this%klg_(2,lag))*xx3*yy4*zz2 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(4,lag),this%klg_(2,lag))*xx4*yy4*zz2
c  k = 3
      + esplfld_(this%ilg_(1,lag),this%jlg_(1,lag),this%klg_(3,lag))*xx1*yy1*zz3 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(1,lag),this%klg_(3,lag))*xx2*yy1*zz3 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(1,lag),this%klg_(3,lag))*xx3*yy1*zz3 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(1,lag),this%klg_(3,lag))*xx4*yy1*zz3 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(2,lag),this%klg_(3,lag))*xx1*yy2*zz3 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(2,lag),this%klg_(3,lag))*xx2*yy2*zz3 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(2,lag),this%klg_(3,lag))*xx3*yy2*zz3 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(2,lag),this%klg_(3,lag))*xx4*yy2*zz3 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(3,lag),this%klg_(3,lag))*xx1*yy3*zz3 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(3,lag),this%klg_(3,lag))*xx2*yy3*zz3 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(3,lag),this%klg_(3,lag))*xx3*yy3*zz3 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(3,lag),this%klg_(3,lag))*xx4*yy3*zz3 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(4,lag),this%klg_(3,lag))*xx1*yy4*zz3 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(4,lag),this%klg_(3,lag))*xx2*yy4*zz3 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(4,lag),this%klg_(3,lag))*xx3*yy4*zz3 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(4,lag),this%klg_(3,lag))*xx4*yy4*zz3  
c  k = 4
      + esplfld_(this%ilg_(1,lag),this%jlg_(1,lag),this%klg_(4,lag))*xx1*yy1*zz4 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(1,lag),this%klg_(4,lag))*xx2*yy1*zz4 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(1,lag),this%klg_(4,lag))*xx3*yy1*zz4 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(1,lag),this%klg_(4,lag))*xx4*yy1*zz4 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(2,lag),this%klg_(4,lag))*xx1*yy2*zz4 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(2,lag),this%klg_(4,lag))*xx2*yy2*zz4 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(2,lag),this%klg_(4,lag))*xx3*yy2*zz4 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(2,lag),this%klg_(4,lag))*xx4*yy2*zz4 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(3,lag),this%klg_(4,lag))*xx1*yy3*zz4 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(3,lag),this%klg_(4,lag))*xx2*yy3*zz4 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(3,lag),this%klg_(4,lag))*xx3*yy3*zz4 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(3,lag),this%klg_(4,lag))*xx4*yy3*zz4 &
      + esplfld_(this%ilg_(1,lag),this%jlg_(4,lag),this%klg_(4,lag))*xx1*yy4*zz4 &
      + esplfld_(this%ilg_(2,lag),this%jlg_(4,lag),this%klg_(4,lag))*xx2*yy4*zz4 &
      + esplfld_(this%ilg_(3,lag),this%jlg_(4,lag),this%klg_(4,lag))*xx3*yy4*zz4 &
      + esplfld_(this%ilg_(4,lag),this%jlg_(4,lag),this%klg_(4,lag))*xx4*yy4*zz4
   ENDDO
...
!-----------------------------------------------------------------
    ...
    IF ( this%rank_ .EQ. 2 ) THEN
      ALLOCATE(this%splfld_(this%ldims_(1)*this%ldims_(2)) )
      reshape(this%splfld_,(/this%ldims_(1),this%ldims_(2)/) )
      ALLOCATE(this%esplfld_(this%ldims_(1)*(this%ldims_(2)+2*nzg)) )
      reshape(this%esplfld_,(/this%ldims_(1),this%ldims_(2)+2*nzg/) )
    ELSE IF ( this%rank_ .EQ. 3 ) THEN
      ALLOCATE(this%splfld_(this%ldims_(1)*this%ldims_(2)*this%ldims_(3)) )
      reshape(this%splfld_,(/this%ldims_(1),this%ldims_(2),this%ldims_(3)/) )
      ALLOCATE(this%esplfld_(this%ldims_(1)*this%ldims_(2)*(this%ldims_(3)+2*nzg)) )
      reshape(this%esplfld_,(/this%ldims_(1),this%ldims_(2),this%ldims_(3)+2*nzg/) )
    ENDIF
    ...
    ALLOCATE(this%zetax_(this%ldims_(1)) )
    ...
    ALLOCATE(this%zetay_(this%ldims_(2)) )

    ALLOCATE(this%wrkl_(this%maxint_))
    ...
    ALLOCATE(this%zetaz_(this%ldims_(3)) )
    ENDIF
...
!-----------------------------------------------------------------
    ...
    IF ( ALLOCATED (this%splfld_   ) ) DEALLOCATE(this%splfld_)
    ...
    IF ( ALLOCATED (this%zetax_) ) DEALLOCATE(this%zetax_)
    ...
    IF ( ALLOCATED (this%zetay_) ) DEALLOCATE(this%zetay_)
    ...
    IF ( ALLOCATED (this%zetaz_) ) DEALLOCATE(this%zetaz_)
...
!-----------------------------------------------------------------
    ...
    DO j = 1, this%rank_
      this%dxi_(j) = real(ldims_(j)-1,kind=GP)/ ( xbnds_(j,2) - xbnds_(j,1) )
    ENDDO

    CALL GPSplineInt_MatInvQ(ldims_(1),ax_,bx_cx_px_,betx_,xxx_,zetax_)
    CALL GPSplineInt_MatInvQ(ldims_(2),ay_,by_cy_py_,bety_,xxy_,zetay_)
    IF ( this%rank_ .GT. 2 ) THEN
    CALL GPSplineInt_MatInvQ(ldims_(3),az_,bz_cz_pz_,betz_,xxz_,zetaz_)
    ENDIF
...
!-----------------------------------------------------------------
    ...
    CLASS(GPSplineInt)                        :: this
    INTEGER      ,INTENT   (IN)              :: n
    REAL(KIND=GP),INTENT(INOUT),DIMENSION(*) :: beta,sixth,twotrd
    REAL(KIND=GP)                            :: beta,sixth,twotrd
...
!-----------------------------------------------------------------
    ...
    REAL(KIND=GP),INTENT   (IN),DIMENSION(*)  :: tmp1,tmp2
    INTEGER                                   :: jm,km
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    DO k=1,nz
      ...
      DO j=1,ny
        jm = j-1
        tmp2 (1,j,k) = field(1+jm*nx+km*nxy)*this%betx_(1)
        tmp2(nx,j,k) = field(nx+jm*nx+km*nxy)
      ENDDO
    ENDDO
!
    DO i=2,nx-2
      DO k=1,nz
        ...
        DO j=1,ny
           jm = j-1
           tmp2    (i,j,k) =  &
           ( field(i+jm*nx+km*nxy) - this%ax_(i)*tmp2(i-1,j,k) )*this%betx_(i)  &
          tmp2    (nx,j,k) = tmp2(nx,j,k) - this%xxx_(i-1)*tmp2(i-1,j,k)
        ENDDO
      ENDDO
    ENDDO
!
...
!  ** n-1 **
        tmp2   (nx-1,j,k) = &
        (field(nx-1+jm*nx+km*nxy) - this%ax_(nx-1)*tmp2(nx-2,j,k))*this%betx_(nx-1)
        tmp2     (nx,j,k) = tmp2(nx,j,k) - this%xxx_(nx-2)*tmp2(nx-1,j,k)
!  ** n  **
        tmp2(nx,j,k) = (tmp2(nx,j,k) - tmp2(nx-1,j,k)*this%zetax_) &
                       * this%betx_(nx)
!  Backsubstitution phase :
       tmp2(nx-1,j,k) = tmp2(nx-1,j,k) - this%gamx_(nx)*tmp2(nx,j,k)
      ENDDO
    ENDDO
!
    DO  i=nx-2,1,-1
      DO j=1,ny
        DO k=1,nz
        tmp2(i,j,k) = tmp2(i,j,k) &
                      - this%gamx_(i+1)*tmp2(i+1,j,k) - this%px_(i)*tmp2(nx,j,k)
        ENDDO
      ENDDO
    ENDDO
...
!  Compute the j-equation (coefficients in y) :
    DO k=1,nz
      km = k-1
      DO i=1,nx
        tmp2(i,1,,k) = field(i+km*nxy)*this%bety_(1)
        tmp2(i,ny,k) = field(i+(ny-1)*nx+km*nxy)
      ENDDO
    ENDDO
!
    DO j=2,ny-2
      ...
      DO k=1,nz
         ...
         DO i=1,nx
           tmp2    (i,j,k) = &
           ( field(i+jm*nx+km*nxy) - this%ay_(j)*tmp2(i,j-1,k) )*this%bety_(j)
           tmp2   (i,ny,k) = tmp2(i,ny,k) - this%xxy_(j-1)*tmp2(i,j-1,k)
         ENDDO
       ENDDO
    ENDDO
!  ** n-1 **
    DO k=1,nz
      ...
      DO i=1,nx
        tmp2    (i,ny-1,k) = &
        (field(i+(ny-1)*nx+km*nxy) - this%ay_(ny-1)*tmp2(i,ny-2,k))*this%bety_(ny-1)
        tmp2      (i,ny,k) = tmp2(i,ny,k) - this%xxy_(ny-2)*tmp2(i,ny-1,k)
!  ** n  **
        tmp2(i,ny,k) = (tmp2(i,ny,k) - tmp2(i,ny-1,k)*this%zetay_) &
                       * this%bety_(ny)
!  Backsubstitution phase :
      tmp2(i,ny-1,k) = tmp2(i,ny-1,k) - this%gamy_(ny)*tmp2(i,ny,k)
      ENDDO
    ENDDO
!
    DO  j=ny-2,1,-1
      DO i=1,nx
        DO k=1,nz
          tmp2(i,j,k) = tmp2(i,j,k)  &
                         - this%gamy_(j+1)*tmp2(i,j+1,k) - this%py_(j)*tmp2(i,ny,k)
        ENDDO
      ENDDO
    ENDDO
!
...
! carry out computation in z:
    CALL gpcomm_%Transpose(field,tmp2,this%ldims_,3,tmp1)
    nx = this%nd_(3)
    ny = this%nd_(2)
    nz = this%ldims_(3)
    nxy = nx*ny
    ...
    DO i=2,nx-2
      DO k=1,nz
        ...
        DO j=1,ny
          ...
          tmp2(nx+jm*nx+km*nxy) = tmp2(nx+jm*nx+km*nxy) - this%xxx_(i-1)*this%tmp2(i-1+jm*nx+km*nxy)
        ENDDO
      ENDDO
    ENDDO
!
...
!  Transpose back to standard coords:
    ...
    CALL gpcomm_%Transpose(field,tmp2,idims,3,tmp1)
