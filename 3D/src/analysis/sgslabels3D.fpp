!=================================================================
      PROGRAM MAIN3D
!=================================================================
! SGSNT: computes the complete SGS_i terms to
! serve as labels in training. This app must be 
! built with the truncated grid size desired.

! It first reads the SGSi* terms from the 
! SGSN2NT app. Then it computes the <N>(<u>,<u>)
! and <N>(<u>,<th>) components of the SGS terms,
! subtracts the SGS* terms and outputs the 
! complete SGS terms at the tuncated resultion.
!
! This app also computes the Sum_i <u>_i SGS_i
! and <th> SGS_th terms and outputs them at
...
! Auxiliary variables
      ...
      COMPLEX(KIND=GP), ALLOCATABLE, TARGET, DIMENSION (:,:,:) :: SGS1,SGS2,SGS3,SGSth
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
      ...
      ALLOCATE( SGS1(nzt,nyt,itsta:itend))
      ALLOCATE( SGS2(nzt,nyt,itsta:itend))
      ALLOCATE( SGS3(nzt,nyt,itsta:itend))
      ALLOCATE( SGSth(nzt,nyt,itsta:itend))
...
!       Read binary data
        ...
        IF ( dolabels ) THEN ! compute SGS terms
          ...
          SGS1 = C4 - SGS1
          SGS1 = SGS1 * rmp ! normalize
          CALL fftp3d_complex_to_real(plancrt,SGS1,R1)
          CALL io_write(1,odir,trim("SGS1_T"),ext,planiot,R1)

          CALL sgs  %sgsv(vx,vy,vz,C1,C2,C3,2, C4)
          SGS2 = C4 - SGS2
          SGS2 = SGS2 * rmp ! normalize
          CALL fftp3d_complex_to_real(plancrt,SGS2,R2)
          CALL io_write(1,odir,trim("SGS2_T"),ext,planiot,R2)

          CALL sgs  %sgsv(vx,vy,vz,C1,C2,C3,3, C4)
          SGS3 = C4 - SGS3
          SGS3 = SGS3 * rmp ! normalize
          CALL fftp3d_complex_to_real(plancrt,SGS3,R3)
          CALL io_write(1,odir,trim("SGS3_T"),ext,planiot,R3)

          ! Now compute u_i SGS^i injection scalars and output:
          !   Note: Real SGSi have already been normalized in R1, R2, R3
          C1 = vx; C1 = C1 * rmp
          CALL fftp3d_complex_to_real(plancrt,C1,R4)
          R1 = R1 * R4 ! vx SGSx; R1 accumulates injection energy
          ...
          R3 = R3 * R4 ! vz SGSz
          R1 = R1 + R3 ! accumulate
          CALL io_write(1,odir,'uSGSinj_T',ext,planiot,R1)
        ...
        ! Energy component:
          ...
          C1 = th; C1 = C1 * rmp
          CALL fftp3d_complex_to_real(plancrt,C1,R2)
          R1 = R1 * R2 ! th  SGSth
          CALL io_write(1,odir,'thSGSinj_T',ext,planiot,R1)
...
! End of MAIN3D
      ...
      DEALLOCATE( R4,R5,R6 )
      DEALLOCATE( SGS1,SGS2,SGS3,SGSth )
