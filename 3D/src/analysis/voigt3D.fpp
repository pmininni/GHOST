...
! Reads from the external file 'voigt.inp' the 
...
!$omp parallel do if (kend-ksta.lt.nth) private (i)
      ...
      ! Compute normalized energy dissipation field/2nu:
!$omp parallel do if (kend-ksta.ge.nth) private (j,i)
      DO k = ksta,kend
!$omp parallel do if (kend-ksta.lt.nth) private (i)
        DO j = 1,ny
...
!
      ...
      REAL   (KIND=GP)                                           :: av(100),sk(100),ku(100),g5(100),vr(100),w6(100)
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
           DO j = 1,ny
              DO k = 1,nz
                 vx(k,j,i) = vx_in(k,j,i)
                 vy(k,j,i) = vy_in(k,j,i)
                 vz(k,j,i) = vz_in(k,j,i)
                 th(k,j,i) = th_in(k,j,i)
              END DO
           END DO
        END DO
      ...
      n = n + 1; sfld(n) = 'dissv' 
      CALL skewflat(dissv,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
...
!     CALL io_write(1,odir,'dthdy',ext,planio,R2)
      ...
      n = n + 1; sfld(n) = 'dthdy' 
      CALL skewflat(R2,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
...
!     CALL io_write(1,odir,'dthdz',ext,planio,R3)
      ...
      n = n + 1; sfld(n) = 'dthdz' 
      CALL skewflat(R3,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
...
!     CALL io_write(1,odir,'gradth',ext,planio,R1)
      ...
      dissp = dissp * (gparams%kappa * tmp * tmp)
      if ( gparams%prtbin .eq.1 ) then
      CALL io_write(1,odir,'dissp',ext,planio,dissp)
      endif
      fnout = trim(odir) // '/' // 'dissppdf.' // ext // '.txt'
      n = n + 1; sfld(n) = 'dissp' 
      CALL skewflat(dissp,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
      ...
      n = n + 1; sfld(n) = 'Rig' 
      CALL skewflat(Rig,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
      ...
      n = n + 1; sfld(n) = 'bflux' 
      CALL skewflat(bf,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
      ...
      n = n + 1; sfld(n) = 'gamf' 
      CALL skewflat(Gamf,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
      ...
      n = n + 1; sfld(n) = 'ommag' 
      CALL skewflat(ommag,nx,ny,knz,av(n),sk(n),ku(n),g5(n),w6(n),vr(n),s3,s4,s5,s6)
...
!     CALL io_write(1,odir,'rdvzdx',ext,planio,R1)
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ! Write statistical data to files:
      IF ( myrank.EQ.0 ) THEN
