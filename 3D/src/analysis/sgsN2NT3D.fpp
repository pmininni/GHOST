!=================================================================
      PROGRAM MAIN3D
!=================================================================
! SGSN2NT: computes a variety of quantities from 
! GHOST DNS data that are truncated to a smaller 
! grid to be used for training data. This should
! be build with size of the DNS simulation data.
!
! This app also computes *part* of the SGS 
! label data: SGSi* = <u.Grad u_i> = <N(u,u)>.
! The complete SGS terms are computed from
!   SGS_i = <N>(<u>,<u>) - <N(u,u)>,
! and the first summand of this expression
! is computed in a second app, SGSNT, that
! reads the SGSi* data on the truncated grid,
...
! Auxiliary variables
      ...
      CHARACTER(len=64)   :: ext1,sfprefv(3),sfprefth
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
      ...
      DO k = 1,3
        ...
        ELSE
          write(sfprefv(k),"(A3,I1,A)") vsdspref, k,"_T"
        ENDIF
      ENDDO
        write(sfprefth,"(A3,A)") thsdspref, "_T"
...
!         write(*,*) 'lesbouss: calling sgsvx...'
          ...
          IF ( commtrunc .NE. MPI_COMM_NULL ) THEN
            CALL io_write(1,odir,trim(sfprefv(1)),ext,planiot,RT1)
          ENDIF

          CALL sgs  %sgsv(vx,vy,vz,C1,C2,C3,2, C4) ! N(u,u)_2
          CALL trunc(C4, n, nt, trtraits%ktrunc, 1, C1, CT1) 
          CALL fftp3d_complex_to_real(plancrt,CT1,RT1)
          IF ( commtrunc .NE. MPI_COMM_NULL ) THEN
            CALL io_write(1,odir,trim(sfprefv(2)),ext,planiot,RT1)
          ENDIF

          CALL sgs  %sgsv(vx,vy,vz,C1,C2,C3,3, C4) ! N(u,u)_3
          CALL trunc(C4, n, nt, trtraits%ktrunc, 1, C1, CT1) 
          CALL fftp3d_complex_to_real(plancrt,CT1,RT1)
          IF ( commtrunc .NE. MPI_COMM_NULL ) THEN
...
! End of MAIN3D
      ...
      SUBROUTINE bouss_lestrain(tr,istat,vx,vy,vz,th)
!-----------------------------------------------------------------
...
!     write(*,*) 'bouss_lestrain: irankpar=', irankpar, ' ... 1 '
      ...
      CALL trunc(vy, n, nt, tr%ktrunc, 1, tr%C3, tr%CT1)  ! vy, normalize
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
        CALL io_write(1,tr%odir,'vy_T',ext,tr%planiot,tr%RT1)
      ENDIF

      CALL trunc(vz, n, nt, tr%ktrunc, 1, tr%C3, tr%CT1)  ! vz, normalize
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
        CALL io_write(1,tr%odir,'vz_T',ext,tr%planiot,tr%RT1)
      ENDIF

      CALL Strain(vx,vy,vz, 1, 1, 1, tr%C2, tr%C1)   ! S11, normalize
      ...
      CALL Strain(vx,vy,vz, 1, 2, 1, tr%C2, tr%C1)   ! S12, normalize
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 0, tr%C3, tr%CT1) 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL Strain(vx,vy,vz, 2, 2, 1, tr%C2, tr%C1)   ! S22, normalize
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 0, tr%C3, tr%CT1) 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL Strain(vx,vy,vz, 2, 3, 1, tr%C2, tr%C1)   ! S23, normalize
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 0, tr%C3, tr%CT1) 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL Strain(vx,vy,vz, 3, 3, 1, tr%C2, tr%C1)   ! S33, normalize
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 0, tr%C3, tr%CT1) 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 1, tr%C3, tr%CT1) ! trunc and normalize
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 0, tr%C3, tr%CT1) ! trunc , don't normalize 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)! trunc , don't normalize
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
        CALL io_write(1,tr%odir,'dSdy_T',ext,tr%planiot,tr%RT1)
      ENDIF

      CALL derivk3(tr%C3, tr%C1, 3)                     ! dS/dz
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 1, tr%C3, tr%CT1) ! trunc , don't normalize 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      CALL trunc(tr%C1, n, nt, tr%ktrunc, 1, tr%C3, tr%CT1) ! trunc, normalize 
      CALL fftp3d_complex_to_real(tr%plancrt,tr%CT1,tr%RT1)
      IF ( tr%commtrunc .NE. MPI_COMM_NULL ) THEN
      ...
      ENDIF
      write(*,*) 'bouss_lestrain: irankpar=', irankpar, ' done.'
#endif

      END SUBROUTINE bouss_lestrain
