...
!                               - N (bu Hu + bs Hs) \hat{z} + nu Del^2 v
!              dbu/dt + v.Grad bu  = - Nu v_z + kappa  Del^2 bu
!              dbs/dt + v.Grad bs  = - Ns v_z + kappa  Del^2 bs
!
!              ds_i/dt + v.Grad s_i = kappa_i Del^2 s_i 
!                                             i = 1, ..., numpassive
!              Div v = 0,
!               
!              where Hu is the heaviside fuction, Hs = 1-Hu, Ns and 
!              Nu are buoyancy freqs, nu is viscosity, kappa is 
!              diffusivity.
!
!              State ordering is:
...
!               +fz(k,j,i))*rmp
!                th(k,j,i) = C20(k,j,i)+dt*(bkappa*th1(k,j,i)+C7(k,j,i) &
!               +fth1(k,j,i))*rmp
!                th2(k,j,i) = C21(k,j,i)+dt*(bkappa1*th1(k,j,i)+C8(k,j,i)  &
!               +fth2(k,j,i))*rmp
