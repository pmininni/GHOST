...
module moist_mod
  ...
  type, public  :: BSTraits
    ...
    real(kind=GP) :: c0param       = 0.0_GP  ! diffusivity
    real(kind=GP) :: c1param       = 0.0_GP  ! diffusivity
...
!$  use threads
    ...
    complex(kind=GP), pointer, dimension(:,:,:) :: fth0, fth1,th0, th1
    ...
    if ( .not. this%binit_ ) then
      stop 'MOISTSolver::dud: Solver not initialized'
    endif
    ...
    th0  => uin(this%activsc   )%ccomp
    th1  => uin(this%activsc+1 )%ccomp

    fx   => uf (this%VELOCITY  )%ccomp
    fy   => uf (this%VELOCITY+1)%ccomp
    fz   => uf (this%VELOCITY+2)%ccomp
    fth0 => uf (this%activsc   )%ccomp
    fth1 => uf (this%activsc+1 )%ccomp
...
!     end do
    ...
    call advect3(vx,vy,vz,th0,C7) ! -(v.Grad) th0
    call advect3(vx,vy,vz,th1,C8) ! -(v.Grad) th1
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
    ...
    call laplak3(th0,th0)          ! Del^2 th
    call laplak3(th0,th1)          ! Del^2 th
...
!             call sctrans (C20,C7,ext,0)
!             call sctpara (C20,C7,ext,0)
!             call sctperp (C20,C7,ext,0)

!             call sctrans (C21,C8,ext,1)
!             call sctpara (C21,C8,ext,1)
!             call sctperp (C21,C8,ext,1)
!          endif
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
    ...
    do k = 1,nz
      if ((kn2(k,j,i).le.kmax).and.(kn2(k,j,i).ge.tiny)) then
        ...
        dudt(this%TEMP0)%ccomp(k,j,i) = bkappa*th0 + C7(k,j,i) + fth0(k,j,i)
        dudt(this%TEMP1)%ccomp(k,j,i) = bkappa*th1 + C8(k,j,i) + fth1(k,j,i)
      else
...
!               +fz(k,j,i))*rmp
!                th(k,j,i) = C20(k,j,i)+dt*(bkappa*th0(k,j,i)+C7(k,j,i) &
!               +fth0(k,j,i))*rmp
!                th1(k,j,i) = C21(k,j,i)+dt*(bkappa1*th1(k,j,i)+C8(k,j,i)  &
!               +fth1(k,j,i))*rmp
...
!          END DO
  ...
  subroutine global_impl(this, uin, uf, t) 
    ...
    integer         , intent(in)                :: t
    complex(kind=GP), pointer, dimension(:,:,:) :: fth0, fth1,fx,fy,fz,vx,vy,vz,th0, th1
    ...
    call pscheck(th0,fth0,t,dt)

    call derivk3(th0,c1,1)
    call derivk3(th0,c2,2)
    call derivk3(th0,c3,3)
    call maxabs(c1,c2,c3,rmq,2)

    call tbouss(vx,vy,vz,th,t,dt,0.0_GP,bvfreq)
    call maxabs(vx,vy,vz,rmp,0)

    ! call mpscheck2(th0,fth0,th1,fs1,t,dt)   ! TODO this include th1 and th2 instaed of 0 and 1
