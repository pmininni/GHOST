...
CONTAINS
  ...
  function init_forcing_from_file(infile,workspace) result(new_object)
    ...
    do i = 1,fnmb
      ...
      case ('null_fv')
        allocate( forceNull_fv    :: New_object(i)%force )
      case ('tg_fv')
        allocate( forceTg_fv      :: new_object(i)%force )
      case ('abc_fv')
        allocate( forceAbc_fv     :: new_object(i)%force )
      case ('random_fv')
        allocate( forceRandom_fv  :: new_object(i)%force )
      ! Electromotive forcing functions --------
      case ('null_fb')
        allocate( forceNull_fb    :: new_object(i)%force )
      case ('random_fb')
        allocate( forceRandom_fb  :: new_object(i)%force )
      ! Active scalar forcing functions --------
      case ('null_fas')
        allocate( forceNull_fas   :: new_object(i)%force )
      case ('puff_fas')
        allocate( forcePuff_fas   :: new_object(i)%force )
      case ('random_fas')
        allocate( forceRandom_fas :: new_object(i)%force )
      ! Passive scalar forcing functions ------- 
      case ('null_fs')
        allocate( forceNull_fs    :: new_object(i)%force )
      case ('puff_fs')
