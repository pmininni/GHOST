...
! in this file.
      ...
      IF (stat.eq.0) THEN
         ...
         IF (j.le.n/4) THEN
            DO i = 1,n
               R1(i,j) = -u0*kdn/(COSH(2*pi*kdn*(float(j)-1)/float(n)))**2 &
                         -u0*corr*kup*SIN(2*pi*kup*(float(i)-1)/float(n))
            END DO
         ELSE IF ((j.ge.n/4+1).and.(j.le.3*n/4)) THEN
            DO i = 1,n
               R1(i,j) = u0*kdn/(COSH(2*pi*kdn*(2*float(j)-float(n)+1) &
                         /float(n)))**2-u0*corr*kup*SIN(2*pi*kup       &
                         *(float(i)-1)/float(n))
            END DO
         ELSE IF (j.ge.3*n/4+1) THEN
            DO i = 1,n
               R1(i,j) = -u0*kdn/(COSH(2*pi*kdn*(float(j)-float(n))    &
                         /float(n)))**2-u0*corr*kup*SIN(2*pi*kup       &
                         *(float(i)-1)/float(n))
            END DO
         ENDIF
...
! Runge-Kutta step 2
         ...
         CALL laplak2(C1,C5)
         CALL laplak2(C2,C6)
         CALL poissonb0(C1,C2,C7,b0)
         CALL poissonb0(C6,C2,C2,b0)
